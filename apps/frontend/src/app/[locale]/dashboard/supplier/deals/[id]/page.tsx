...
const NEXT_LABEL: Record<string, { en: string; ar: string }> = {
  AWARDED:     { en: 'Mark In Progress', ar: 'بدء التنفيذ' },
  IN_PROGRESS: { en: 'Mark Delivered',   ar: 'تم الشحن' },
};
...
export default function SupplierDealDetailPage() {
  const locale = useLocale();
  ...
  const startConversation = async () => {
    ...
    try {
      const res = await api.post('/conversations/start', { participantCompanyId: deal.buyerId });
    ...
    <DashboardLayout>
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        <div className="flex items-center gap-3">
          <Link href={`/${locale}/dashboard/supplier/deals`} className="btn-ghost p-2">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            ...
            {deal.buyer && (
              <div>
                <p className="text-slate-500 text-xs">{ar ? 'المشتري' : 'Buyer'}</p>
                <p className="font-medium text-slate-800">{ar ? deal.buyer.nameAr : deal.buyer.nameEn}</p>
              </div>
            )}
          </div>
        ...
        {quote && (
          <div className="card">
            <h2 className="font-semibold text-slate-800 mb-4">{ar ? 'تفاصيل عرضك' : 'Your Quote Details'}</h2>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
              <div>
