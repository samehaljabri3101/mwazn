      ...
      <div className="max-w-3xl">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">{ar ? 'اعرض منتجاتك للمشترين في موازن' : 'Showcase your products to buyers on Mwazn'}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="card space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">{ar ? 'رمز التعريف الداخلي للمنتج' : 'Internal product identifier'}</p>
              </div>
            </div>
          ...
          <div className="card space-y-4">
            ...
            <p className="text-xs text-slate-500">{ar ? 'أضف مواصفات المنتج (اسم المواصفة — القيمة)' : 'Add key-value specification pairs for the product'}</p>
            {specs.map((spec, i) => (
              <div key={i} className="grid grid-cols-5 gap-2 items-center">
                <div className="col-span-2">
                  <Input value={spec.key} onChange={(e) => updateSpec(i, 'key', e.target.value)} placeholder={ar ? 'المواصفة (مثال: المادة)' : 'Property (e.g. Material)'} />
                </div>
                <div className="col-span-2">
                  <Input value={spec.value} onChange={(e) => updateSpec(i, 'value', e.target.value)} placeholder={ar ? 'القيمة (مثال: PVC)' : 'Value (e.g. PVC)'} />
                </div>
